...
class MainWindow:
    ...
    def processRButton(self):
        self.lock=False
        filename=self.myio.nextfilename()
    ...
    def _DrawCircle(self,moving=True,update=False):
        ...
        if self.isInnerCircle:
            self.inner_d.set(d)
            self.inner_x.set(self.x1+d//2-1)
            self.inner_y.set(self.y1+d//2-1)
        else:
            self.outside_d.set(d)
            self.outside_x.set(self.x1+d//2-1)
            self.outside_y.set(self.y1+d//2-1)
    ...
    def __init__(self):
        ...
        self.showPicture("image/rabbit640480.jpg")
