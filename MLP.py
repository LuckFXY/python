...
def test_mlp(learning_rate=0.1,L1_reg=0.00,L2_reg=0.0001,n_epochs=15,
    ...
    while(epoch <n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if(iter+1)%validation_frequency==0:
                ...
                #当前验证误差比之前的都小，则更新best_validaiton_loss，以及best_itereeeeeee
                #并且在testdata上进行测试
                if this_validation_loss < best_validation_loss:
                    ...
                    best_params=ret_model_params()
                    print (
                        'good job!test error %f %%' %
                        (test_score*100.)
                    )
            if patience<iter:
                done_looping=True
