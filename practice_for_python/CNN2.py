...
#才能存到GPU memory中，只能为float类型，data_y是类别，所以最后又转换成int
    ...
    print 'try to open params'
    if os.path.isfile('kaggle\\params.py'):
        ...
        print '-.-cannont find params.py'
    
    rval=[(train_set_x,train_set_y),(valid_set_x,valid_set_y),params]
    return rval          
#===========================LeNet-5 demo================================
def evaluate_lenet5(learning_rate=0.01,n_epochs=20,
    ...
    print (
        'Best train score of %f %% with validation performance %f %%'%
        (min_cost*100.,best_validation_loss*100.)
    )
